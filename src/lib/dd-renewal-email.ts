 ...
 *
 * Initial payment (1st April): EGU + County (if applicable) + first month membership
 * Monthly = floor((subscription + locker) / 12) to nearest penny
 * First month = total annual - (11 x monthly)
 */
export function calculateDDSchedule(
  ...
  // Monthly payment = floor(total / 12) to nearest penny
  const monthlyPayment = Math.floor((totalAnnualMembership / 12) * 100) / 100;

  // First month picks up the rounding remainder
  const firstMonthPayment = +(totalAnnualMembership - 11 * monthlyPayment).toFixed(2);

  // Initial collection = EGU + County + first month
  const initialCollectionTotal = +(englandGolfFee + countyFee + firstMonthPayment).toFixed(2);
          ...
          <tr>
            <td style="padding: 15px 30px;">
              ...
              <p style="margin: 0 0 10px 0; font-size: 14px; color: #333; line-height: 1.6;">
                Your first collection will include any applicable England Golf and county affiliation fees
